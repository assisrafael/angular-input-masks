(function() {
	...
	var ieMasks = {
		...
		'ES': [{mask: new StringMask('000.000.00-0')}],
		...
		'PR': [{mask: new StringMask('00000000-00')}],
	...
	function getMask(uf, value) {
		...
		if (_uf === 'SP' && /^P/i.test(value)) {
			console.log('>> SPECIAL MASK SP <<');
			return ieMasks.SP[1].mask;
		}
		var masks = ieMasks[uf];
		var i = 0;
		while(masks[i].chars && masks[i].chars < value.length && i < masks.length - 1) {
			i++;
		}
		return masks[i].mask;
	}
	...
	.directive('uiBrIeMask',function() {
		function clearValue (value, state) {
			if(!value) {
				return value;
			}
		...
		function applyIEMask (value, uf, ctrl) {
			...
			if (uf && uf.toUpperCase() === 'SP' && /^p/i.test(value)) {
				return 'P'+formatedValue.trim().replace(/[^0-9]$/, '');
			}
